...
class TestNWP500BinarySensor:
    ...
    def test_binary_sensor_operation_busy(
        ...
        from custom_components.nwp500.binary_sensor import create_binary_sensor_descriptions
        
        descriptions = create_binary_sensor_descriptions()
        op_busy_desc = next(d for d in descriptions if d.key == "operation_busy")
    ...
    def test_binary_sensor_false(
        ...
        descriptions = create_binary_sensor_descriptions()
        freeze_desc = next(d for d in descriptions if d.key == "freeze_protection_use")
