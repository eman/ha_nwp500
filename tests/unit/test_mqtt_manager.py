...
async def test_subscribe_device(manager, mock_mqtt_client, mock_device):
    ...
    assert mock_mqtt_client.subscribe_device_status.call_args[0][0] == mock_device
    
    mock_mqtt_client.subscribe_device_feature.assert_called_once()
    assert mock_mqtt_client.subscribe_device_feature.call_args[0][0] == mock_device
...
async def test_send_command_queued(manager, mock_mqtt_client, mock_device):
    ...
    error = AwsCrtError(code=0, name="AWS_ERROR_MQTT_CANCELLED_FOR_CLEAN_SESSION", message="Clean session")
...
@pytest.mark.asyncio
async def test_request_status_consecutive_timeouts(manager, mock_mqtt_client, mock_device):
